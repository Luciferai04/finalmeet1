...
class ServeClientBase(object):
 ...
 """
    with self.lock:
    if self.frames_np[int((self.timestamp_offset - self.frames_offset)
                          * self.RATE):].shape[0] > 25 * self.RATE:
 ...
 """
    ...
    if len(segments) > 1 and self.get_segment_no_speech_prob(
            segments[-1]) <= self.no_speech_thresh:
    for s in segments[:-1]:
    text_ = s.text
    ...
    last_segment = self.format_segment(
        ...
        self.timestamp_offset + min(duration,
                                    self.get_segment_end(segments[-1])),
        self.current_out,
        completed=False
    )
    ...
    # audio thats not yet transcribed so, capturing the time when it was
    # repeated for the first time
    ...
    if not self.text or self.text[-1].strip(
    ).lower() != self.current_out.strip().lower():
