...
class WhisperModel:
 ...
 """
    ...
    options = TranscriptionOptions(
        ...
        temperatures=(
            temperature if isinstance(
                temperature, (list, tuple)) else [temperature]
        ),
        initial_prompt=initial_prompt,
        prefix=prefix,
        suppress_blank=suppress_blank,
    ...
    break
        ...
        # all failed, select the result with the highest average log
        # probability
    decode_result = max(
    ...
    # a better segmentation algorithm based on VAD should be able to replace
    # this.
 ...
 """
    ...
    detected_language_info.setdefault(
        ...
        # projected languages for all segments is used to determine the
        # language.
    language = max(
        detected_language_info,
...
def merge_punctuations(
        alignment: List[dict], prepended: str, appended: str) -> None:
